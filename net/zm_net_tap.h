 ...
 *
 * @brief 生命周期由 ZmTapContext 管理：Get() 获取 → 使用 → Drop() 回收 → 空闲栈复用。
 * @brief 务必保证在自己的业务链中, 只要调用了Drop,后续无论哪个线程都不要再继续使用这个ZM_TAP_CTX了,
 *        因为多线程的原因,你释放的ZM_TAP_CTX会回到事件循环中回收且可能被其他线程继续利用,
 *        所以你的业务中的ZM_TAP_CTX已不再是之前保存的内容了,继续操作会破坏新的所有者的数据
 *        拿到ZM_TAP_CTX后,你的异步事件继续传递它后,成功的回调和失败的回调都要处理它的生命周期,否则它会成为僵尸
 * @brief 扩容时 _slot 回指指针会被同步修正，保证外部持有的槽位引用不悬空。
 */
