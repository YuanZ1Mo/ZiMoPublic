 ...
 *  - 写盘模型:**每会话一个写线程**,dlLoop 经有界指令队列(WRITE/TRUNCATE/
 *    WRITE_META/FINISH/ABORT)投递;队列字节上限 downloadQueueMaxBytes(默认 64MB),
 *    触顶 ABORT(.part/.meta 保留可续传);写阻塞由 dlLoop CancelIoEx 解除;
 *    FINISH 由写线程确认"队列排空+句柄关闭+改名成功"后回调--残余未落盘类缺陷结构性不存在; 被取消的写按真实原因回执(不报"写盘失败").
 *  - 解析器:fail-closed--chunk size 行必须全 hex,数据块后必须 CRLF;
 ...
 *  - 生命周期:连接代数(epoch)守卫,重连窗口内旧连接回调被忽略; 文件打开/侧车读取由 StartDownload 提交客户端工作池执行(不占调用方线程);
